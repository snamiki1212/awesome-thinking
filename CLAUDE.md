...
**階層と呼称（3段）**: 思考の知識は次の3階層で呼び分ける。包含関係は **思考ツール ⊃ 系統 ⊃ タイプ**。数えるときは「2系統」「4タイプ」と言う。経緯: [adr/20260604-tier-terminology](./adr/20260604-tier-terminology/README.md)
...
- **🔍 レンズ系 / ⚙️ プロセス系**: 思考ツールを2つに大別した中間階層＝**系統**（1＋3）。4タイプの違いを説明するときの大枠として使い、図・見出しではこの絵文字で統一する。
  - **🔍 レンズ系**＝🧠 メンタルモデル … 状況を**見る・捉える**静的な視点（手順なし／別軸）。
  - **⚙️ プロセス系**＝🌊 思考法 ⊃ 🧩 フレームワーク ⊃ ⚡ 思考術 … 実際に**考えを進める・手を動かす**道具（抽象→具体の入れ子）。
