...
> 💡 コーディングエージェント（Claude Code 等）で作業する場合は、この手順を案内するスキル `add-thinking-concept`（`.claude/skills/add-thinking-concept/`）が使えます。「〇〇を追加したい」と依頼すれば自動で参照されます。
...
4. **インデックスを2か所**更新する（漏れやすい）。
   - 該当ジャンルの `README.md` の一覧表に1行追加する。
   - トップ `README.md` の該当ジャンル表にも1行追加する。
