...
type: guideline
title: 思考の分類と定義
description: "「思考」の知識を4タイプに分けるこのリポジトリ内での定義・判定基準と、収録の受け入れ要件の正本。"
timestamp: 2026-06-05T15:33:55+09:00
tags: [分類, 運用ルール]
---
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 🧠 メンタルモデル | `thinking-mental-models/` | 何が起きているか | なし（前提・視点） | 高 | 概念 | 状況を眺めるとき・常時 | 世界を理解する「レンズ」 |
| 🧩 思考フレームワーク | `thinking-frameworks/` | どう整理するか | あり（枠が明確） | 低 | 枠組み・型 | 情報を整理・分析するとき | 情報を構造化する「テンプレート」 |
...
4タイプは、まず **2グループ** に大別できます（総称＝**思考ツール**、包含は 思考ツール ⊃ グループ ⊃ タイプ。呼称は [`adr/20260604-tier-terminology`](../adr/20260604-tier-terminology/README.md)）。🔍 **レンズ**＝🧠 メンタルモデルは手順を持たない「見る・捉える」視点。⚙️ **プロセス**＝🌊 思考法 ⊃ 🧩 フレームワーク ⊃ ⚡ 思考術 は「考えを進める・手を動かす」道具で、抽象→具体の入れ子です。レンズはプロセスの全体に、見え方のレンズとして効きます。
...
flowchart TB
    subgraph T["思考ツール"]
        ...
        subgraph L["🔍 レンズ（見る・捉える）"]
            MM["🧠 メンタルモデル<br/>例: 第一原理 / 複利 / 機会費用"]
        end
        subgraph P["⚙️ プロセス（進める・動かす／抽象→具体）"]
            subgraph TM["🌊 思考法〈抽象・大きな構え〉<br/>例: ロジカルシンキング"]
                direction TB
                subgraph FW["🧩 フレームワーク〈型〉<br/>例: ロジックツリー / MECE"]
                    SK["⚡ 思考術〈具体・その場の一手〉<br/>例: 抽象化 / リフレーミング"]
                end
            end
        end
    end
...
> 🔍 レンズ（メンタルモデル）は手順を持たない視点、⚙️ プロセスの3つ（思考法 ⊃ フレームワーク ⊃ 思考術）は「抽象 → 具体」の1本のグラデーション。両者は別グループなので色で分けています。4タイプを L1〜L4 の通し番号で1列に並べない理由は [`adr/20260602-no-layer-numbering/`](../adr/20260602-no-layer-numbering/README.md) を参照。
