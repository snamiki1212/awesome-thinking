...
思考ツールを収録・更新・廃止した項目は、`[AT-0001: 第一原理思考](../thinking-mental-models/first-principles.md)` の形式で ID・名称・詳細ページへの相対リンクを付ける。
ID は名称や配置が変わっても同じ項目を追えるため、変更の対象を取り違えずに参照できる。
...
タグは Semantic Versioning に従う `vMAJOR.MINOR.PATCH` を使う。
変更履歴導入前の基準版は `v0.1.0` とする。

- `MAJOR` は既存の URL・分類・利用方法を変える変更で上げる。
- `MINOR` は思考ツールや利用者向け資料を追加する変更で上げる。
- `PATCH` は既存の内容の修正・補足で上げる。

1. `CHANGELOG.md` の `Unreleased` の下に `## [vMAJOR.MINOR.PATCH] - YYYY-MM-DD` を追加し、公開する項目を移す。空の `Unreleased` は残す。
2. この変更を `main` にマージする。
