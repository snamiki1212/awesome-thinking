...
| `id` | Extension | 思考ツールでは**必須** | string | 思考ツールを一意に識別する不変の ID。形式は `AT-0001` |
...
思考ツールの詳細ファイルには `id` を必須とする。
ID は `AT-0001` 形式の連番で、項目名・スラッグ・タイプ・配置を変えても変更しない。
採番・一覧表示・検査の規則は [docs/thinking-ids.md](../thinking-ids.md) を正本とする。
