...
混同されがちな4タイプを、同じお題「**アプリの解約が増えている**」にどう関わるかで対比します。

| タイプ | ひとことで | 「解約が増えた」ならこう関わる | 一覧 |
| --- | --- | --- | --- |
| 🧠 メンタルモデル | 見え方を変える**レンズ** | 「複利」で捉え*解約が解約を呼ぶ悪循環では?*と見る | [→](./mental-models/README.md) |
| 🧩 フレームワーク | 整理する**型** | なぜなぜ分析の枠で*なぜ? →なぜ?…*と原因を掘る | [→](./frameworks/README.md) |
| 🧭 思考法 | 攻める**方向・姿勢** | *そもそも本当に増えてる?*と前提自体を疑う | [→](./thinking-methods/README.md) |
| 🛠 思考術 | その場の**一手** | 問いを*「続けたくなる理由は?」*に言い換える | [→](./thinking-skills/README.md) |

4つは横並びではなく、**思考法の中にフレームワーク、その中で思考術を打つ**という入れ子の関係です。メンタルモデルだけは別軸で、全体に「見え方のレンズ」として効きます。
...
入れ子の3つは「抽象（大きな構え）→ 具体（その場の手）」のグラデーション。メンタルモデルは黄色で示すとおり、その梯子には乗らない**別軸のレンズ**です。

さらに詳しい比較表・読み解き方は ➡ **[docs/taxonomy.md](./docs/taxonomy.md)**
